...
import { EventsService, type Queryable, type UpsertResult } from '@/services/events.service';
...
export async function runJob(job: SyncJob): Promise<SyncJobResult> {
  const db = getDbPool();
...
export async function startJob(job: SyncJob): Promise<StartedSyncJob> {
  const db = getDbPool();
  const runner = new SyncRunner({
    db,
    eventsService: new EventsService(db),
...
export class SyncRunner {
  ...
  private async finishJob(
    ...
    const finishedAt = this.now();
    const errorSummary =
      upsertResult.errors.length > 0 ? summarizeErrors(upsertResult) : undefined;
    ...
    if (this.cache) {
      try {
        await this.cache.del(`events:list:*${job.sportSlug}*`);
      } catch {
